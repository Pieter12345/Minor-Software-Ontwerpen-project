      ...
      Pass {	
         ...
         float4 frag(vertexOutput input) : COLOR
         {
            ...
            float3 ambientLighting = UNITY_LIGHTMODEL_AMBIENT.rgb;
 			...
 			if (attenuation*(max(0.0, (dot(normalDirection, lightDirection)))) > _Treshold) {temp = normalize((dot(normalDirection, lightDirection))*(_Intensity,_Intensity,_Intensity));}
 			else {temp = (0,0,0);}

			float3 diffuseReflection = temp * _LightColor0.rgb * _Color.rgb; 
			
            //texture maps
            float4 tex = tex2D(_MainTex, input.tex.xy * _MainTex_ST.xy + _MainTex_ST.zw);
            float3 texLighting = tex.xyz * _LightColor0.rgb;
             
            return float4((diffuseReflection + ambientLighting) * texLighting.xyz * _Color.xyz, 1.0);
         }
